    ...
    try {
        ...
        // Fallback payload if the entire geofencing cycle fails
        // ==========================================
        // 🛑 SYNTHETIC DISASTER MOCK (REMOVE FOR PRODUCTION)
        // ==========================================
        weatherData.isExtremeEvent = true;
        weatherData.alertMessage = "IMD RED ALERT: Severe Cyclonic Storm approaching the Saurashtra coast. Wind speeds exceeding 120km/h expected. Secure all farm equipment immediately.";
        weatherData.isRainImminent = true;
        weatherData.sprayRecommendation = "⚠️ SEVERE WEATHER WARNING: Do not apply chemical treatments. Evacuate fields.";
        
        // Force the next 3 hours on the timeline to show 100% rain probability
        if (weatherData.hourlyForecast && weatherData.hourlyForecast.length > 3) {
            weatherData.hourlyForecast[0].rain_chance = 100;
            weatherData.hourlyForecast[1].rain_chance = 100;
            weatherData.hourlyForecast[2].rain_chance = 90;
        }
        // ==========================================
        return {
        currentTemp: 0,
